...
def runCustomers(recipe, price, ingredients):
    ...
    for i in range(len(customers)):
        ...
        if price > price_limit:
            print(f"Customer {i + 1} looked at the price and walked away. Too expensive")
        elif abs(recipe["cheese"] - cheesy_pref) > 1:
            print(f"Customer {i + 1} didnt like the cheese amount. They left")
        elif abs(recipe["hotSauce"] - spice_pref) > 1:
            print(f"Customer {i + 1} didnt like the spice level. They left")
        else:
